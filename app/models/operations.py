...
def addSport(cursor: object, data: dict):
    ...
    query: str = """
    INSERT OR IGNORE INTO sports (sports_name)
    VALUES (?)
    """
    cursor.execute(query, data["sportsName"])
...
def addCompetition(cursor: object, data: dict):
    ...
    VALUES (?, ?)
    """
    cursor.execute(query, (data["competitionId"], data["competitionName"],))
...
def addStage(cursor: object, data: dict):
    ...
    VALUES (?, ?, ?)
    """
    cursor.execute(query, (data["id"], data["name"], data["ordering"],))
...
def checkMatchExists(cursor: object, home_team_id, away_team_id, timeVenue, dateVenue):
    ...
    cursor.execute(query, (home_team_id, away_team_id, timeVenue, dateVenue,))
